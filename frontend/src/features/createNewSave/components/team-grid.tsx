...
export function TeamGrid({ teams, handleSelectTeam }: TeamGridProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {teams.flatMap((team) =>
                <TeamCard key={team.id} teamData={team} leagueName={team.league.name} handleSelectTeam={handleSelectTeam}/>
            )}
        </div>
    );
}
